<template>
    <footer class="bg-blue-100 p-8">
        <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
                <h2 class="font-semibold text-lg">服务</h2>
                <ul class="mt-4 space-y-2">
                    <li>
                        <a href="https://example.com/qq-group" class="text-black hover:text-blue-500">QQ群</a>
                    </li>
                    <li>
                        <a href="https://example.com/online-map" class="text-black hover:text-blue-500">在线地图</a>
                    </li>
                </ul>
            </div>

            <div>
                <h2 class="font-semibold text-lg">文档</h2>
                <ul class="mt-4 space-y-2">
                    <li>
                        <a href="https://example.com/docs" class="text-black hover:text-blue-500">文档站</a>
                    </li>
                    <li>
                        <a href="https://example.com/terms" class="text-black hover:text-blue-500">玩家守则</a>
                    </li>
                    <li>
                        <a href="https://example.com/privacy" class="text-black hover:text-blue-500">加入教程</a>
                    </li>
                </ul>
            </div>

            <div>
                <h2 class="font-semibold text-lg">合作</h2>
                <ul class="mt-4 space-y-2">
                    <li>
                        <a href="https://qm.qq.com/q/rq9qBgy2DS" class="text-black hover:text-blue-500">MistMC招聘群</a>
                    </li>
                </ul>
            </div>

            <div>
                <h2 class="font-semibold text-lg">寻找我们</h2>
                <ul class="mt-4 space-y-2">
                    <li>
                        <a href="https://play.mcmod.cn/sv20187990.html" class="text-black hover:text-blue-500">MCMOD(找服玩)</a>
                    </li>
                    <li>
                        <a href="https://www.minebbs.com/threads/1-20-4.27037/r"
                            class="text-black hover:text-blue-500">MineBBS论坛</a>
                    </li>
                    <li>
                        <a href="https://space.bilibili.com/3546689776388263"
                            class="text-black hover:text-blue-500">Bilibili</a>
                    </li>
                </ul>
            </div>
        </div>

        <div class="border-t mt-8 pt-4 flex justify-between items-center">
            <div class="flex items-center space-x-2">
                <img src="http://iph.href.lu/900x900" alt="Logo" class="w-12 h-12" />
                <span class="font-semibold text-lg">MistMC</span>
            </div>
            <p class="text-sm text-gray-600">&copy; 2024 MistMC 禁止盗用 制作者: mc506lw</p>
        </div>
    </footer>
</template>

<script>
export default {
    name: 'Footer',
};
</script>

<style scoped></style>
